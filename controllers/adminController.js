...
module.exports = {
    ...
    addBank: async (req, res) => {
        try {
            const { name, nameBank, nomorRekening, image } = req.body;
            await Bank.create({
                name,
                nameBank,
                nomorRekening,
    ...
    editBank: async (req, res) => {
        try {
            const { id, editNameBank, editNomorRekening, editName } = req.body;
            const bank = await Bank.findOne({ _id: id });
            if (req.file == undefined) {
                bank.nameBank = editNameBank;
                bank.nomorRekening = editNomorRekening;
                bank.name = editName;
                await bank.save();
        ...
        try {
            ...
            for (let i = 0; i < item.imageId.length; i++) {
                Image.findOne({ _id: item.imageId[i]._id })
                    .then((result) => {
                        fs.unlink(path.join(`public/${result.imageUrl}`));
                        result.remove();
                    })
                    .catch((err) => {
                        req.flash("alertMessage", `${error.message}`);
    ...
    actionConfirm: async (req, res) => {
        ...
        try {
            ...
            await booking.save();
            req.flash("alertMessage", "Payment Confirmed");
            req.flash("alertStatus", "success");
